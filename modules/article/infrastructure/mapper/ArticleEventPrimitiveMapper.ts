...
export type ArticleEventPrimitive = {
  articleId: string;
  authorId: string;
  type: EventType;
  version: number;
...
export const ArticleEventPrimitiveMapper = {
  toPrimitive(event: ArticleEvent): ArticleEventPrimitive {
    const base = {
  ...
  fromPrimitive(serialized: ArticleEventPrimitive): ArticleEvent {
    const articleId = new ArticleId(serialized.articleId);
    const authorId = new AuthorId(serialized.authorId);
    ...
    switch (serialized.type) {
      ...
      case EVENT_TYPE.PUBLISH:
        return ArticleEventFactory.publish({
          articleId,
          authorId,
          version: serialized.version,
          eventDate: occurredAt,
      ...
      case EVENT_TYPE.ARCHIVE:
        return ArticleEventFactory.archive({
          articleId,
          authorId,
          version: serialized.version,
          eventDate: occurredAt,
